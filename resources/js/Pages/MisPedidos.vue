...
const puedeDevolver = (pedido) => {
    if (!['entregado'].includes(pedido.estado)) return false;
                ...
                <div v-for="pedido in pedidos" :key="pedido.idPedido"
                    ...
                    <div class="p-6 flex flex-col md:flex-row justify-between items-center gap-4 cursor-pointer hover:bg-gray-50"
                        ...
                        <div class="flex items-center gap-4">
                            <span class="text-[#010cf7] transition-transform duration-300"
                                :class="{ 'rotate-180': pedidoAbierto === pedido.idPedido }">
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            <span
                                :class="['px-3 py-1 text-xs font-bold rounded-full uppercase', getEstadoConfig(pedido.estado).css]">
                                {{ getEstadoConfig(pedido.estado).label }}
                            </span>
                        </div>
                    </div>

                    <div v-if="pedidoAbierto === pedido.idPedido"
                        class="border-t border-gray-100 bg-gray-50 p-6 animate-fade-in-down">
                        <h4 class="text-sm font-bold text-gray-700 mb-4 uppercase text-center md:text-left">Artículos:
                        </h4>
                                ...
                                <tbody>
                                    <tr v-for="item in pedido.items" :key="item.id"
                                        ...
                                        <td class="py-3 font-medium text-gray-800">
                                            {{ item.nombre_articulo || 'Producto ID: ' + item.idProducto }}
                                        </td>
                                        ...
                                        <td class="py-3 text-right font-bold">
                                            {{ formatearPrecio(item.precioUnitario * item.cantidadProducto) }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="mt-8 flex flex-wrap justify-end gap-3 border-t pt-4">
                            ...
                            <button v-if="puedeDevolver(pedido)" @click="abrirModalDevolucion(pedido.idPedido)"
                            ...
                            <div v-if="mostrarModal"
                                ...
                                <div
                                    class="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in-up">
                                    <div class="p-6">
                                        <h3 class="text-xl font-bold text-gray-800 mb-2">¿Por qué quieres devolver este
                                            pedido?</h3>
                                        <p class="text-sm text-gray-500 mb-4">Cuéntanos qué ha pasado para que podamos
                                            ayudarte mejor.</p>

                                        <textarea v-model="form.motivo"
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#010cf7] focus:border-[#010cf7] p-3 text-sm"
                                            rows="4"
                                            placeholder="Ej: El producto es equivocado / El producto llegó dañado..."></textarea>
                                        ...
                                        <div class="mt-6 flex justify-end gap-3">
                                            ...
                                            <button @click="enviarDevolucion" :disabled="form.processing"
                                                class="bg-[#010cf7] text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-blue-700 disabled:opacity-50">
