...
const groupedProducts = computed(() => {
  ...
  for (const item of props.products) {
    ...
    let key
    if (parseFloat(espesor) === 4.2)      key = 'Serie 10.1.C'
    else if (parseFloat(espesor) <= 3.0)  key = 'Serie 10.1.B'
    else                                  key = 'Serie 10.1.A'
...
<template>
  <Head title="Serie 10.1 Discos para Sierra Múltiple Con Dentones Sin Limitador - HECOMA" />
