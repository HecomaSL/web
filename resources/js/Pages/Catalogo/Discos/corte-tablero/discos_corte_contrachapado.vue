...
const groupedProducts = computed(() => {
  const groups = { 'Serie 1.4.A': [], 'Serie 1.4.B': [], 'Serie 1.4.C': [] }
  for (const item of props.products) {
    const { espesor } = parseRef(item.referencia)
    let key
    if (parseFloat(espesor) === 4.0)      key = 'Serie 1.4.C'
    else if (parseFloat(espesor) <= 3.0)  key = 'Serie 1.4.B'
    else                                   key = 'Serie 1.4.A'
