<script setup>
    ...
    const tablaAgrupada = computed(() => {
        ...
        props.productos.forEach(prod => {
          const match = prod.referencia.match(/F20-(\d{3})R(\d{2,3})(A|B)(M|H)/);
          ...
          if (match) {
            ...
            if (!grupos[baseRef]) {
              const rMap = { 5:  11, 8:  14, 10: 16,  12: 18, 15: 22, 20: 27, 25: 32, 30: 38 };
  ...
  <MainLayout>
    <section class="bg-[#010cf7] py-6 mb-15">
      <div class="container mx-auto px-6">
        <h1 class="text-white text-3xl md:text-4xl font-bold text-center uppercase tracking-wide">F20 Fresa radio 1/4 cóncavo</h1>
      </div>
    </section>
    ...
    <section class="container mx-auto px-6 mb-10">
      <div class="max-w-6xl mx-auto space-y-6 text-gray-700 leading-relaxed text-lg">
        <h2 class="text-[#010cf7] text-3xl font-bold mb-6">Características de nuestra fresa para radio cóncavo 1/4:</h2>
        ...
        <p>Fresa para realizar radios. Fabricada con ángulo axial, permitiendo buen acabado y suavidad en el corte. El cuerpo de la herramienta está fabricado en aleación de acero de alta resistencia. Las placas de metal duro (carburo de tungsteno) son soldadas mediante aleación trimetálica. Con estos materiales y una fabricación mediante CNC de alta precisión, obtenemos gran rendimiento, estabilidad en la calidad del corte y una gran duración de la herramienta.</p>
          ...
          <table class="w-full text-sm text-left border-collapse bg-white">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(fila, idx) in tablaAgrupada" :key="idx" class="hover:bg-blue-50/50 transition-colors">
                ...
                <td v-if="$page.props.auth.user" class="px-4 py-4 text-center"><button v-if="fila.md" @click="agregarAlCarrito(fila.md)" class="hover:scale-125 transition-transform">🛒</button><span v-else>---</span></td>
                ...
                <td v-if="$page.props.auth.user" class="px-4 py-4 text-center bg-[#fafcfe]/40"><button v-if="fila.hss" @click="agregarAlCarrito(fila.hss)" class="hover:scale-125 transition-transform">🛒</button><span v-else>---</span></td>
              </tr>
            </tbody>
          </table>
        </div>
