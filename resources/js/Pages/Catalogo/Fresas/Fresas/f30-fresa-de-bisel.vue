    ...
    const tablaAgrupada = computed(() => {
      ...
      props.productos.forEach(prod => {
        ...
        if (match) {
          ...
          const valorB   = parseInt(match[2]);
          const valorAlpha = parseInt(match[3]);
          const tipo     = match[4];
          const material = match[5];
          const baseRef = prod.referencia.slice(0, -1);

          if (!grupos[baseRef])
            grupos[baseRef] = {medidas: { D: diametro, B: valorB, d: '50', Z: '4', tipo: tipo, Alpha: valorAlpha }, md: null, hss: null };
