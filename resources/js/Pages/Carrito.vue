...
const aplicarCupon = async () => {
    ...
    try {
        const res = await axios.post('/cupones/validar', {
            codigo: codigoCupon.value.trim().toUpperCase(),
                        ...
                        <div v-for="item in cart.items" :key="item.id"
                            ...
                            <div class="text-right min-w-[120px]">
                                ...
                                <button @click="cart.removeFromCart(item.id)"
                                    class="text-red-500 text-xs uppercase font-bold hover:underline mt-1">Eliminar</button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100 sticky top-24">
                            ...
                            <div class="mb-4">
                                <div v-if="!cart.cupon" class="flex gap-2">
                                    <input v-model="codigoCupon" type="text" placeholder="CÓDIGO"
                                        class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-[#0000BB]"
                                        @keyup.enter="aplicarCupon" />
                                    <button @click="aplicarCupon" :disabled="cuponCargando || !codigoCupon"
                                        class="bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-gray-700 disabled:opacity-50 transition-colors">
                                ...
                                <div v-if="cart.cupon"
                                    ...
                                    <span class="text-green-700 font-bold uppercase">
                                        ✓ {{ cart.cupon.nombre }}
                                    </span>
                            ...
                            <Link href="/tramitar-pedido"
                                class="w-full bg-[#0000BB] text-white py-4 rounded-lg font-bold uppercase hover:bg-blue-800 transition-all shadow-lg text-center block">
                                Tramitar Pedido
                            </Link>
