...
// Badge reactivo para el carrito
const totalItems = computed(() => cartStore.totalProductos || 0);

/**
 * COMPROBACIÓN DE USUARIO
 * Forzamos la reactividad accediendo a page.props.auth.user
 */
const user = computed(() => page.props.auth?.user);

/**
 * HELPER PARA ADMIN
 * Comprueba si el usuario es admin (flexible a 'SI', 'si' o true)
 */
const isAdmin = computed(() => {
  const status = user.value?.admin;
  return status === 'SI'; 
});
...
const toggleSubMenu = (itemLabel) => { 
  openSubMenu.value = openSubMenu.value === itemLabel ? null : itemLabel; 
};
  ...
  <header class="bg-white text-[#0009ad] shadow-lg relative z-50">
    <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <div class="text-2xl font-bold uppercase tracking-wider">
        <Link href="/">
          <img src="https://hecoma.com/wp-content/uploads/2025/04/logohecomaincoyprecom-1.png" alt="Logo"
            class="w-48 lg:w-64">
        </Link>
      </div>
      ...
      <ul class="hidden lg:flex items-center space-x-8 font-medium">
        <li v-for="(items, key) in menus" :key="key" class="relative group" 
            @mouseenter="openMenu = key" @mouseleave="openMenu = null">
          ...
          <transition name="fade">
            <ul v-if="openMenu === key" class="absolute left-0 bg-white text-blue-900 rounded-md shadow-xl py-2 w-64 border">
              <li v-for="item in items" :key="item.label" class="relative" 
                  @mouseenter="openSubMenu = item.label" @mouseleave="openSubMenu = null">
                
                <Link :href="item.url" class="flex justify-between items-center px-4 py-2 hover:bg-blue-100 text-sm">
                  {{ item.label }}
                  <svg v-if="item.subitems" class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <li class="relative group" @mouseenter="isUserMenuOpen = true" @mouseleave="isUserMenuOpen = false">
          <button class="bg-[#010cf7] text-white px-4 py-1.5 rounded-full flex items-center gap-2 font-bold hover:bg-blue-800 transition-all shadow-md">
            ...
            {{ user ? (user.nombre?.split(' ')[0] || 'Mi Perfil') : 'Mi Cuenta' }}
          </button>

          <transition name="fade">
            <ul v-if="isUserMenuOpen" class="absolute right-0 bg-white text-blue-900 rounded-md shadow-xl py-2 w-52 border">
              
              <template v-if="!user">
                <li><Link href="/login" class="block px-4 py-2 hover:bg-blue-100 text-sm font-bold">Iniciar Sesión</Link></li>
                <li><Link href="/register" class="block px-4 py-2 hover:bg-blue-100 text-sm">Registrarse</Link></li>
              </template>

              <template v-else>
                <li v-if="isAdmin">
                  <Link href="/admin" class="block px-4 py-2 hover:bg-blue-100 text-sm font-bold text-[#010cf7] border-b border-gray-100">
                    ⚙️ Panel de Admin
                  </Link>
                </li>
                ...
                <li class="border-t mt-2 pt-2">
                  <Link href="/logout" method="post" as="button" class="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 text-sm font-bold">
                    Cerrar Sesión
                  </Link>
                </li>
              </template>
        ...
        <ul class="px-6 py-4 space-y-4">
          ...
          <li v-if="user" class="bg-blue-900 p-4 rounded-lg">
            <div class="flex justify-between items-center mb-4">
              <div>
                ...
                <p class="font-bold text-lg">{{ user.nombre }}</p>
              </div>
              <Link href="/carrito" class="relative p-2 bg-blue-800 rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.112 11.213a.45.45 0 0 1-.445.494H3.734a.45.45 0 0 1-.445-.494l1.112-11.213m15.356 0a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25m15.356 0h-15.356" />
                </svg>
                <span v-if="totalItems > 0" class="absolute -top-1 -right-1 bg-red-600 text-[10px] px-1.5 py-0.5 rounded-full font-bold border border-blue-900">{{ totalItems }}</span>
              </Link>
            </div>
            <div class="flex flex-col gap-2">
              <Link :href="isAdmin ? '/admin/dashboard' : '/dashboard'" class="text-sm bg-white text-[#010cf7] text-center py-2 rounded-md font-bold">
          ...
          <li v-for="(items, key) in menus" :key="key">
            <div class="flex justify-between items-center border-b border-blue-800">
              <Link :href="getTitleUrl(key)" class="py-2 text-lg font-bold capitalize flex-1">{{ key.replace(/([A-Z])/g, ' $1') }}</Link>
              <button @click="toggleMenu(key)" class="p-2">
                <svg class="w-5 h-5 transition-transform" :class="{ 'rotate-180': openMenu === key }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M19 9l-7 7-7-7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
              </button>
            </div>
              ...
              <li v-for="item in items" :key="item.label">
                <div class="flex justify-between items-center py-1">
                  ...
                  <button v-if="item.subitems" @click="toggleSubMenu(item.label)" class="p-1">
                    <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': openSubMenu === item.label }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path d="M19 9l-7 7-7-7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                  </button>
                </div>
                <ul v-if="item.subitems && openSubMenu === item.label" class="pl-4 mt-1 space-y-1">
                  <li v-for="sub in item.subitems" :key="sub.label">
                    <Link :href="sub.url" class="block text-blue-200 text-xs py-1 hover:text-white">{{ sub.label }}</Link>
                  </li>
                </ul>
              </li>
            </ul>
