...
<template>
    <div class="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
        <div v-if="isOpen" 
             class="mb-4 w-[350px] h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col transition-all duration-300">
        ...
        <button 
            @click="isOpen = !isOpen"
            class="w-14 h-14 bg-[#010cf7] hover:bg-[#003399] text-white rounded-full shadow-lg flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
        >
            <svg v-if="!isOpen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
