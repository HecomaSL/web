...
class CuponController extends Controller
{
    public function validar(Request $request)
    {
        ...
        $cupon = Cupon::where('nombre', $codigo)
                      ->where('activo', true)
                      ->first();
        ...
        if ($cupon->minimo_pedido !== null && $subtotal < $cupon->minimo_pedido) {
            return response()->json([
                'message' => 'El pedido mínimo para este cupón es de ' . number_format($cupon->minimo_pedido, 2) . '€'
            ], 400);
        }
        ...
        return response()->json([
            ...
            'cupon' => [
                'nombre' => $cupon->nombre,
                'tipo'   => $cupon->tipo,
                'valor'  => (float) $cupon->valor, // Forzamos float
                'minimo_pedido' => $cupon->minimo_pedido ? (float) $cupon->minimo_pedido : null
            ]
        ]);
    }
}