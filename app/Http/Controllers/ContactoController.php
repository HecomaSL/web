...
class ContactoController extends Controller {
    ...
    public function enviar(Request $request) {
        ...
        $datosValidados = $request->validate([
            ...
            'email.email'      => 'Introduzca una dirección de correo válida.',
            'asunto.required'  => 'Debe indicar un asunto.',
            'mensaje.required' => 'El cuerpo del mensaje no puede estar vacío.',
            'imagen.image'     => 'El archivo debe ser una imagen.',
            'imagen.max'       => 'La imagen no puede ocupar más de 2MB.',
        ]);

        try {
            // 2. Envío del correo
            // Cambia 'info@hecoma.com' por la dirección donde quieras recibir los mensajes
            Mail::to('dptocomercial@hecoma.com')->send(new ContactoMail($datosValidados));

            // 3. Respuesta a Inertia (redirección con mensaje de éxito)
            return back()->with('success', '¡Su mensaje ha sido enviado correctamente!');

        } catch (\Exception $e) {
            // En caso de error (fallo de SMTP, etc.)
